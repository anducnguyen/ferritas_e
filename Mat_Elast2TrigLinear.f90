SUBROUTINE Mat_Elast2TrigLinear(eNum)
   USE PrecTypes
   USE GlobalData
   IMPLICIT NONE

   ! -------------------------------------------------------------------------
   ! NAME:   Mat_Elast2TrigLinear
   ! PARENT: Matrix
   ! STATUS: Current
   ...
   !         and store them into the global stiffness matrix.
   ...
   DO jr=1,dir      ! repetition for dimension numbers: 2 or 3.
      ...
      DO jc=1,dir         ! repetition for dimension numbers: 2 or 3.
         ...
         IF (row > 0) then
            ...
            IF(column > 0) then ! should be stored on the left-hand side
               Km(row, column) = Km(row, column) + Ktrig(ir,ic, jr,jc)   
            ELSE                ! should be moved to the right-hand side by multiplying the known quantities
               Fv(row) = Fv(row) - Ktrig(ir,ic, jr,jc) * u(n(ic), jc)
            END IF
         ...
         ELSE ! When row < 0 row is stored as a negative value.
            ...
            Kmm(-row, dir*(elm(eNum,ic)-1)+jc) = Kmm(-row, dir*(elm(eNum,ic)-1)+jc) + Ktrig(ir,ic, jr,jc)
      ...
      END DO ! <-- jc=1,dir
         ...
         IF (pcode == 3) THEN
            IF (row > 0) THEN
                  Fv(row)   = Fv(row)   + Thtrig(ir,jr,ic) * T(n(ic))               

            ! When the displacement is known and the nodal force is unknown
            ELSE ! row < 0 のとき
                  Fvv(-row) = Fvv(-row) + Thtrig(ir,jr,ic) * T(n(ic))
            END if
         END IF
   ...
   RETURN
END SUBROUTINE Mat_Elast2TrigLinear
